...
var memo = kitty.Trigger{
	...
	Action: func(b *kitty.Bot, m *kitty.Message) {
		...
		b.Reply(m, fmt.Sprintf("%s's memo will be sent to %s when I see them join or post", m.Name, nick))
	},
}
...
var memowatcher = kitty.Trigger{
	...
	Action: func(b *kitty.Bot, m *kitty.Message) {
		...
		for _, v := range memos {
			b.Msg(ircChannel, fmt.Sprintf("%s's memo to %s: %s", v.Sender, m.Name, v.Message))
		}
	},
}
